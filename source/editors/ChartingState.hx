...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In the Chart Editor", StringTools.replace(_song.song, '-', ' '));

		vortex = FlxG.save.data.chart_vortex;
		ignoreWarnings = FlxG.save.data.ignoreWarnings;
		background = new FlxSprite().loadGraphic(Paths.image(daImage + '/' + daBGShit[0]));
		...
		var daFolderUI:String = 'UI';
		var daChart:String = 'chart';
		quant = new AttachedSprite(daFolderUI + '/' + daChart + '/' + 'chart_quant', 'chart_quant');
