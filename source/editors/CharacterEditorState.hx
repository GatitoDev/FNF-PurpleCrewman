...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSky:BGSprite = new BGSprite('backgrounds/' + curStage + '/weebSky', OFFSET_X - (playerXDifference / 2) - 300, 0 - playerYDifference, 0.1, 0.1);
			bgLayer.add(bgSky);
			bgSky.antialiasing = false;

			var repositionShit = -200 + OFFSET_X - playerXDifference;

			var bgSchool:BGSprite = new BGSprite('backgrounds/' + curStage + '/weebSchool', repositionShit, -playerYDifference + 6, 0.6, 0.90);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:BGSprite = new BGSprite('backgrounds/' + curStage + '/weebStreet', repositionShit, -playerYDifference, 0.95, 0.95);
			bgLayer.add(bgStreet);
			bgStreet.antialiasing = false;

			var widShit = Std.int(bgSky.width * 6);
			var bgTrees:FlxSprite = new FlxSprite(repositionShit - 380, -800 - playerYDifference);
			bgTrees.frames = Paths.getPackerAtlas('backgrounds/' + curStage + '/weebTrees');
			...
			var bg:FlxSprite;
			bg = new FlxSprite(-600 + OFFSET_X - playerXDifference, -200).loadGraphic(Paths.image('backgrounds/' + curStage + '/stageback', 'shared'));
			bg.scrollFactor.set(0.9, 0.9);
			bg.updateHitbox();
			bgLayer.add(bg);

			var stageFront:FlxSprite;
			stageFront = new FlxSprite(-650 + OFFSET_X - playerXDifference, 600).loadGraphic(Paths.image('backgrounds/' + curStage + '/stagefront', 'shared'));
